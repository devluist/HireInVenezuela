...
class BaseWriteForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('body',)
        widgets = {
            # usar el traductor pal placeholder  xHACER:    _("body")
            'body': forms.Textarea(attrs={'class':"col-md-12", 'placeholder': "Ingrese el contenido del mensaje..." }),
        }
...
class WriteForm(BaseWriteForm):
    ...
    recipients = CommaSeparatedUserField(label=(_("Recipients"), _("Recipient")), help_text='', widget=forms.TextInput(attrs={'class':"col-md-3",
        'placeholder':"Para:"  # xHACER: _("Recipients")
    }))
    subject = forms.CharField(widget=forms.TextInput(attrs={'class':"col-md-8 col-md-offset-1", 'placeholder':_("subject").capitalize()+"..." }))
...
class FullReplyForm(BaseReplyForm):
    ...
    if allow_copies:
        recipients = CommaSeparatedUserField(
            label=(_("Additional recipients"), _("Additional recipient")), help_text='', required=False, widget=forms.TextInput(attrs={
            'placeholder':"Para:"  # _("Recipients")
        }))
