...
class FolderMixin(NamespaceMixin, object):
    ...
    def get_context_data(self, **kwargs):
        ...
        datos = {
            ...
            'pag_menu': "postman/menu-superior.html"
            'gets': self.request.GET,  # useful to postman_order_by template tag
        }
        if self.request.user.is_authenticated():
            p = get_object_or_404(Perfil, usuario=self.request.user)
            datos['perfil'] = p
...
class DisplayMixin(NamespaceMixin, object):
    ...
    def get_context_data(self, **kwargs):
        ...
        for m in reversed(self.msgs):
            ...
            received = None
        if "obj_cola" in kwargs:
            serv_cola = get_object_or_404(Cola, id=kwargs["obj_cola"])
        else:
            serv_cola = 0
        # xHACER: ponerle acentos a las oraciones
        msj = {
            "1": "Se ha enviado una solicitud al vendedor para cancelar la transaccion.",
            "2": "Se ha enviado una solicitud al comprador para cancelar la transaccion.",
        }
        datos = {
